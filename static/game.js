...
const openCharacterSelectButton = document.getElementById("openCharacterSelectButton");
const backToStartButton = document.getElementById("backToStartButton");
const selectedCharacterName = document.getElementById("selectedCharacterName");
const characterOptions = document.querySelectorAll(".characterSelectOption");
const scoreDisplay = document.getElementById("score");
const highScoreDisplay = document.getElementById("highScore");
const gameOverBox = document.getElementById("gameOverBox");
const gameOverCharacterImage = document.getElementById("gameOverCharacterImage");

const JUMP_DURATION_MS = 900;
const PLAYER_PADDING = 48;
const OBSTACLE_PADDING = 12;
const BIRD_PADDING = 14;
const PLAYER_STAND_HEIGHT = 300;
const PLAYER_DUCK_HEIGHT = 140;
...
const OBSTACLE_MIN_SPAWN_DELAY_MS = 700;
const OBSTACLE_MAX_SPAWN_DELAY_MS = 1800;
...
const CHARACTER_FRAMES = {
    ...
    haya: {
        label: "Mighty Haya",
        frameOne: 'url("/static/haya 1.png")',
        frameTwo: 'url("/static/haya 2.png")',
        duckFrame: 'url("/static/haya 1.png")'
    }
};
...
function applyCharacterFrames() {
    const frames = CHARACTER_FRAMES[selectedCharacter] || CHARACTER_FRAMES.banana;
    document.documentElement.style.setProperty("--runner-frame-1", frames.frameOne);
    document.documentElement.style.setProperty("--runner-frame-2", frames.frameTwo);
    document.documentElement.style.setProperty("--runner-duck-frame", frames.duckFrame);
}
...
function updateCharacterUi() {
    ...
    selectedCharacterName.innerText =
        (CHARACTER_FRAMES[selectedCharacter] || CHARACTER_FRAMES.banana).label;
}

function positionSprites() {
    obstacle.style.left = `${obstacleX}px`;
    obstacle.style.display = obstacleActive ? "block" : "none";
...
function getRandomObstacleDelay() {
    return (
        OBSTACLE_MIN_SPAWN_DELAY_MS +
        Math.random() * (OBSTACLE_MAX_SPAWN_DELAY_MS - OBSTACLE_MIN_SPAWN_DELAY_MS)
    );
}
...
function queueNextObstacle() {
    ...
    obstacleSpawnTimerMs = getRandomObstacleDelay();
}

function resetBird() {
    const obstacleAnchorX = obstacleActive ? obstacleX : window.innerWidth + 120;
    const minBirdX = obstacleAnchorX + obstacle.offsetWidth + BIRD_SPAWN_GAP;
    birdX = Math.max(window.innerWidth + BIRD_EXTRA_GAP, minBirdX + Math.random() * 220);
}

function keepBirdBehindObstacle() {
    if (!birdActive || !obstacleActive) return;
...
function gameOver() {
    ...
    const gameOverImages = {
        banana: "/static/banana cryingg.png",
        spongebob: "/static/spongebob crying.jpg",
        haya: "/static/haya crying.png"
    };

    const gameOverImage = gameOverImages[selectedCharacter];
    if (gameOverImage) {
        gameOverCharacterImage.src = gameOverImage;
        gameOverCharacterImage.classList.remove("hidden");
    } else {
        gameOverCharacterImage.classList.add("hidden");
    }
...
function resetRunState() {
    ...
    score = 0;
    speed = START_SPEED;
    obstacleX = window.innerWidth + 120;
    birdX = window.innerWidth + 1200;
    backgroundOffset = 0;
    lastFrameTime = null;
    scoreAccumulator = 0;
    birdActive = false;
    obstacleActive = true;
    obstacleSpawnTimerMs = 0;

    player.classList.remove("jump", "duck");
    player.style.height = `${PLAYER_STAND_HEIGHT}px`;
    gameOverCharacterImage.classList.add("hidden");
...
function updateSpeed() {
    speed = Math.min(MAX_SPEED, START_SPEED + Math.floor(score / 100) * SPEED_STEP);
}

function checkCollisions() {
    const playerRect = insetRect(player.getBoundingClientRect(), PLAYER_PADDING);
    const obstacleRect = obstacleActive
        ? insetRect(obstacle.getBoundingClientRect(), OBSTACLE_PADDING)
        : null;
    const birdRect = birdActive ? insetRect(bird.getBoundingClientRect(), BIRD_PADDING) : null;

    const hitObstacle = obstacleRect ? rectsOverlap(playerRect, obstacleRect) : false;
...
function updateWorld(deltaMs) {
    ...
    if (obstacleActive) {
        ...
        obstacleSpawnTimerMs -= deltaMs;
        if (obstacleSpawnTimerMs <= 0) {
            spawnObstacle();
        }
    }
    ...
    if (obstacleActive && obstacleX + obstacle.offsetWidth < 0) {
        queueNextObstacle();
        keepBirdBehindObstacle();
    }
...
openCharacterSelectButton.addEventListener("click", showCharacterSelectScreen);
...
document.addEventListener("keydown", (e) => {
    if (!hasGameStarted && !startScreen.classList.contains("hidden") && (e.code === "Space" || e.code === "Enter")) {
        e.preventDefault();
        startGame();
        return;
    }
